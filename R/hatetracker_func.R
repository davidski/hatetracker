...
get_hatetracker_activity <- function(start_date = Sys.Date()) {
  dat <- httr::GET(paste("https://api.hatetracker.io/v1/activity", start_date, sep = "/")) %>%
    httr::content(as = "text") %>% jsonlite::fromJSON(flatten = TRUE) %>%
    tibble::as_data_frame()
  dat$timeline <- purrr::map(dat$timeline, ~ {
    .x$name <- as.POSIXct(.x$name, tz = "GMT")
    purrr::set_names(.x, c("date", "z_score"))
    })
  dat
}
...
get_hatetracker_activity2 <- function(start_date = Sys.Date(), end_date = NULL) {
  ...
  dat$timeline <- purrr::map(dat$timeline, ~ { as_data_frame(.x) %>%
      ...
      dplyr::mutate(date = as.POSIXct((date / 1000), origin = "1970-01-01", tz = "GMT"))})
...
#' @importFrom magrittr '%>%'
#' @importFrom tibble as_data_frame
#' @param hashtag Topic hashtag.
#' @param start_date Beginning of date range. Defaults to three weeks from the current date.
#' @param end_date End of date range. Defaults to the current date.
#' @export
get_hatetracker_mentions <- function(hashtag = NULL, start_date = Sys.Date() - 21, end_date = Sys.Date()) {
